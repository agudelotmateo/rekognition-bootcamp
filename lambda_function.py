    ...
    for inputImages in s3Resource.Bucket(inputBucket).objects.filter(Prefix='People/'):
        ...
        if len(inputFilename):
            ...
            for faceDetail in response['FaceDetails']:
                if deb:
                    print('**Processing face {}/{}'.format(faceCounter, len(response['FaceDetails'])))
                ...
                for emotion in faceDetail['Emotions']:
                    ...
                    if confidence > maxValue:
                        maxValue = confidence
                        detectedEmotion = emotion['Type'].encode('ascii', 'ignore').lower()
                        
                if deb:
                    print('**Identifying face location...')
                faceUpperLeftX = int(faceDetail['BoundingBox']['Left']*inputImageWidth)
                faceUpperLeftY = int(faceDetail['BoundingBox']['Top']*inputImageHeigth)
                faceHeight = int(faceDetail['BoundingBox']['Height']*inputImageHeigth)
                faceWidth = int(faceDetail['BoundingBox']['Width']*inputImageWidth)
                
                if deb:
                    print('**Gathering emoji...')
                emoji = getImage('Emojis/'+detectedEmotion+'.png', inputBucket)
                
                if deb:
                    print('**Resizing emoji...')
                emoji = emoji.resize((faceWidth, faceHeight), Image.ANTIALIAS)
                
                if deb:
                    print('**Pasting emoji...')
                inputImage.paste(emoji, (faceUpperLeftX, faceUpperLeftY), emoji)
                faceCounter += 1
